...
This release adds a `--registry-exclude-image` flag to the daemon to
exclude certain (or all) images from scanning using glob expressions,
allows for registries with self-signed certificates and fixes multiple
bugs.
...
- Allow (all) images to be excluded from scanning
...
contributions to this release, including those who I may have missed
